...
class Trainer(object):
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        mix_reg_cls = self.cfg.model.mix_reg_cls
        if mix_reg_cls:
            pred_cls = logging_output['prediction_cls']
            pred_reg = logging_output['prediction_reg']
            label_cls = logging_output['label_cls']
            label_reg = logging_output['label_reg']
            is_reg = logging_output['is_reg']
        else:
            pred = logging_output['prediction']
            label = logging_output['label']
        if self.cfg.model.moe_attention_full:
            moe_attentions_full = logging_output['moe_attentions_full']
            task_idx = logging_output['task_idx']
        ...
        if mix_reg_cls:
            if self.cfg.model.moe_attention_full:
                return logging_output, pred_cls, pred_reg, label_cls, label_reg, is_reg, moe_attentions_full,task_idx
            else:
                return logging_output, pred_cls, pred_reg, label_cls, label_reg, is_reg
        else:
            return logging_output, pred, label
