...
def progress_bar(
    ...
    sweep_mode: Optional[bool] = False
):
    ...
    if wandb_project:
        bar = WandBProgressBarWrapper(bar, wandb_project, run_name=wandb_run_name,sweep_mode = sweep_mode)
...
class WandBProgressBarWrapper(BaseProgressBar):
    ...
    def __init__(self, wrapped_bar, wandb_project, run_name=None,sweep_mode = False):
        self.wrapped_bar = wrapped_bar
        if wandb is None:
        ...
        if not sweep_mode:
            if run_name.split("_")[0]=="ofl":
                wandb.init(project=wandb_project, reinit=False, name=run_name, mode = "offline")
            else:
